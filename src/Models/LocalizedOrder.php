...
class LocalizedOrder extends ModelWrapper
{
    ...
    {
        //filter all order items with references
        $filteredOrderItemsWithReferences = $orderItems->filter(
            function ($filterOrderItem) {
                if (isset($filterOrderItem->relations['references']) && count(
                        $filterOrderItem->relations['references']
                    )) {
                    return true;
                }
            }
        ...
        //filter all order items with reference type set and referenced to $setOrderItemId
        $filteredOrderItemsWithSetReference = $filteredOrderItemsWithReferences->filter(
            function ($filterOrderItem) use ($setOrderItemId) {
                //filter all order items with reference type set
                $setReferences = $filterOrderItem->relations['references']->filter(
                    function ($reference) {
                        return $reference->referenceType === 'set';
                    }
                );

                //now filter all with the references $setOrderItem
                $concreteReferences = $setReferences->filter(
                    function ($reference) use ($setOrderItemId) {
                        return $reference->referenceOrderItemId == $setOrderItemId;
                    }
                );

                if (count($concreteReferences)) {
                    return true;
                }
            }
        );

        return $filteredOrderItemsWithSetReference;
    }
}
